...
function SearchableRoleSelector({
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'ArrowDown') {
      ...
      if (isOpen && highlightedIndex >= 0 && highlightedIndex < filteredRoles.length) {
        e.preventDefault()
  ...
  return (
    <div className="relative flex flex-col gap-1.5 w-full">
      <label htmlFor={id} className="text-black/75 font-semibold text-xs tracking-wider uppercase">{label}</label>
...
export default function StepTeamSize({ draft, onNext, onBack }: StepProps) {
  // Load draft values
  const initialStatus = draft.team_status === 'LOOKING'
    ? 'solo'
  ...
  const [collaborationStatus, setCollaborationStatus] = useState<'solo' | 'leader' | 'member'>(initialStatus)
  const [membersNeeded, setMembersNeeded] = useState<number>(draft.team_size_needed || 1)
  ...
  const [preferredRoles, setPreferredRoles] = useState<string[]>(() => {
    ...
    if (draft.track) {
      ...
      const match = ROLE_OPTIONS.find(r => r.toLowerCase() === mapped.toLowerCase())
      return match ? [match] : []
    }
    return []
  })
  ...
  const [neededRoles, setNeededRoles] = useState<string[]>(() => {
    if (draft.team_status === 'LOOKING_FOR_MORE' && draft.looking_for_role) {
      const roles = draft.looking_for_role.split(', ')
      const targetSize = draft.team_size_needed || roles.length || 1
      while (roles.length < targetSize) {
        roles.push('')
      }
      return roles.slice(0, targetSize)
    }
    return Array(draft.team_size_needed || 1).fill('')
  })
  
  const [teamInviteCode, setTeamInviteCode] = useState('')
  const [myRole, setMyRole] = useState(draft.team_status === 'COMPLETE' ? draft.looking_for_role || '' : '')
  ...
  const filteredSoloRoles = useMemo(() => {
    ...
    return unselected.filter(role => role.toLowerCase().includes(q)).slice(0, 8)
  ...
  const handleKeyDownSolo = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'ArrowDown') {
      ...
      if (!isOpenSolo) {
        ...
        setHighlightedIndexSolo(prev => (prev < filteredSoloRoles.length - 1 ? prev + 1 : prev))
      }
    } else if (e.key === 'ArrowUp') {
      e.preventDefault()
      if (isOpenSolo) {
      ...
      e.preventDefault()
      if (isOpenSolo && highlightedIndexSolo >= 0 && highlightedIndexSolo < filteredSoloRoles.length) {
        selectSoloRole(filteredSoloRoles[highlightedIndexSolo])
      } else if (soloSearch.trim()) {
        const match = ROLE_OPTIONS.find(r => r.toLowerCase() === soloSearch.trim().toLowerCase())
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onNext({
      team_status: collaborationStatus === 'solo'
        ? 'LOOKING'
        : collaborationStatus === 'leader'
          ? 'LOOKING_FOR_MORE'
          : 'COMPLETE',
      is_available: collaborationStatus !== 'member',
      team_size_needed: collaborationStatus === 'leader' ? membersNeeded : 1,
      looking_for_role: collaborationStatus === 'solo'
        ? preferredRoles.join(', ')
        : collaborationStatus === 'leader'
          ? neededRoles.join(', ')
          : myRole,
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <section className="space-y-5 light-glass rounded-3xl p-6 border border-black/5 shadow-sm">
        ...
        <div className="flex flex-col gap-3">
          <span className="text-black/75 font-semibold text-xs tracking-wider uppercase">Collaboration Status *</span>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {[
              { id: 'solo', label: 'Solo Student', desc: 'Looking to Join a Team', icon: '👤' },
              { id: 'leader', label: 'Team Leader', desc: 'Looking for Members', icon: '👥' },
              { id: 'member', label: 'Team Member', desc: 'Already in a Team', icon: '🔐' },
            ].map(status => (
              <button
                key={status.id}
                type="button"
              ...
              >
                ...
                <div>
                  <div className="font-bold text-xs sm:text-sm">{status.label}</div>
                  <div className={cn(
                    "text-[10px] sm:text-[11px] leading-tight mt-0.5",
                    collaborationStatus === status.id ? "text-white/70" : "text-neutral-500"
                  )}>{status.desc}</div>
                </div>
              </button>
            ))}
        ...
        {collaborationStatus === 'solo' && (
          <div className="space-y-4 pt-2 animate-fade-in">
            ...
            <div className="relative flex flex-col gap-1.5 w-full">
              <label htmlFor="preferred-roles-input" className="text-black/75 font-semibold text-xs tracking-wider uppercase">Preferred Roles (Optional)</label>
              <div className="relative">
                <input
        ...
        {collaborationStatus === 'leader' && (
          <div className="space-y-5 pt-2 animate-fade-in">
            ...
            <div className="flex flex-col gap-2">
              <span className="text-black/75 font-semibold text-xs tracking-wider uppercase">How many members do you need? *</span>
                ...
                {[1, 2, 3, 4, 5].map((size) => (
                  <button
                    ...
                    className={cn(
                      ...
                      membersNeeded === size
                        ? "bg-violet-600 border-violet-600 text-white shadow-violet-200/50"
