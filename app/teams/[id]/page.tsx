...
export default function TeamDetailsPage() {
    ...
    return (
      <div className="p-6 h-[calc(100vh-80px)] overflow-y-auto flex flex-col justify-center items-center">
        <div className="w-full max-w-md bg-white rounded-3xl border border-gray-100 p-8 text-center shadow-lg flex flex-col items-center space-y-4">
          ...
          <button 
            onClick={() => router.back()}
            className="px-6 py-2.5 bg-[#ef4d23] text-white font-bold rounded-xl shadow-md hover:bg-orange-600 transition-colors"
          >
            Go Back
          </button>
        </div>
  ...
  // 1. Compute dynamic team size specs
  const teamSizeNeeded = teammates.reduce((max, m) => Math.max(max, m.team_size_needed || 2), 2)
  const spotsOpen = Math.max(0, teamSizeNeeded - teammates.length)
  
  // 2. Compute majority commitment level
  const commitmentCounts = teammates.reduce((acc: Record<string, number>, m) => {
  ...
  // Badges metadata
  const trackKey = team.looking_for_track || 'OTHER'
  const trackColor = TRACK_COLORS[trackKey as keyof typeof TRACK_COLORS] || TRACK_COLORS.OTHER
  const trackLabel = TRACK_LABELS[trackKey as keyof typeof TRACK_LABELS] || TRACK_LABELS.OTHER

  const commitmentBadgeColor = 
    majorityCommitment === 'LOW' ? 'bg-green-100 text-green-700' :
    majorityCommitment === 'HIGH' ? 'bg-red-100 text-red-700' :
    'bg-amber-100 text-amber-700' // MEDIUM
  const commitmentBadgeLabel = 
    majorityCommitment === 'LOW' ? 'Low commitment' :
    majorityCommitment === 'HIGH' ? 'High commitment' :
    'Medium commitment'

  // Teammates section progress specs
  const progressPercentage = Math.min(100, (teammates.length / teamSizeNeeded) * 100)
  const isFull = teammates.length >= teamSizeNeeded
  const progressColor = isFull ? 'bg-green-500' : 'bg-violet-600'

  // What they're building specs
  const hasTechnologies = !!projectTechnologies && projectTechnologies.trim().length > 0
    ...
    <div className="p-6 h-[calc(100vh-80px)] overflow-y-auto space-y-8 pb-14 text-left max-w-3xl mx-auto">
      ...
      <div className="space-y-3.5">
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {/* Spots Open Badge */}
          {spotsOpen > 0 ? (
            <Badge color="bg-violet-100 text-violet-700">{spotsOpen} {spotsOpen === 1 ? 'spot' : 'spots'} open</Badge>
          ) : (
              ...
              >
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-gray-900 text-sm leading-snug">{member.full_name}</h4>
                ...
                <div className="mt-4 pt-3 border-t border-gray-50 flex items-center justify-between">
                  <span className="text-[9px] font-black text-gray-400 uppercase tracking-wide">Contacts:</span>
