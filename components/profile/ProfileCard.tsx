...
import { getInitials, TRACK_COLORS, TRACK_LABELS } from '@/lib/utils'
...
export default function ProfileCard({ profile, showActions = false }: ProfileCardProps) {
  ...
  const track = profile.track in TRACK_LABELS ? profile.track : 'OTHER'
  const commitmentLevel = profile.commitment_level in COMMITMENT_LABELS
  ...
  const teamStatusPill = () => {
    switch (profile.team_status) {
      case 'LOOKING':
        return <Badge color="bg-green-100 text-green-700">Looking for team</Badge>
      case 'COMPLETE':
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0 pt-0.5">
          <div className="flex items-center gap-2">
            <h3 className="font-bold text-neutral-900 text-lg truncate leading-snug">{fullName}</h3>
            {/* Availability dot */}
            <span
              className={cn(
          ...
          {profile.department && (
            <p className="text-xs font-semibold text-[#ef4d23] uppercase tracking-wider truncate mt-0.5">{profile.department}</p>
          )}
          {profile.gpa != null && (
            <p className="text-xs font-medium text-neutral-400 mt-0.5">GPA {profile.gpa.toFixed(2)}</p>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5">
        <Badge color={TRACK_COLORS[track]}>
          {TRACK_LABELS[track]}
        </Badge>
      ...
      {visibleSkills.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          ...
          {extraSkills > 0 && (
            <span className="text-[12px] text-gray-400 self-center">+{extraSkills} more</span>
          )}
        </div>
      )}
          ...
          <div className="flex gap-2 ml-auto">
            ...
            {profile.whatsapp_number && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 rounded-full bg-[#ef4d23] px-4 py-2 text-xs font-medium text-white hover:bg-[#ef4d23]/90 transition-all shadow-[0_4px_12px_rgba(239,77,35,0.15)]"
              >
